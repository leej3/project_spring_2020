...
# an example of a way to use the read_raw_eeg_file function is as a comment right here:
...
# Picks are the channels to include, start_epochs_seconds are the seconds in the recording where the epoching segmentation will begin, stop_epochs_seconds are the seconds in the recording where the epoching segmentation will end, epoch_duration is the duration of each segment /epoch being created
...
# The plot_epochs_vs_uV function is just a more intuitive name for a plot of epochs created that plots the epochs in regard to amplitude (i.e in uV)
def plot_epochs_vs_uV():
    epochs.plot_image()


# An example of a way to use the above function is as a comment right here:
# epoch_raw_eeg_file(Picks = 'EEG F4-LE', start_epochs_seconds=5, stop_epochs_seconds=50, epoch_duration=2)
# plot_regular_epoched_data()
...
# An example of a way to use the above function is as a comment right here with fmin at 20 and fmax at 30:
